...
public class Glossary extends Activity {
    ...
    public static void SetGlossary(TextView text, String string, Context context, int position, String compare) {
        if (position == 0)
            GLOSSARY_COUNT = 0;
        for (int i = 65; i <= 90; i++ )
            GlossaryStart(text, string, Character.toString ((char) i), compare);
        int color = GlossaryColor();
        text.setBackgroundColor(ContextCompat.getColor(context, R.color.light_yellow));
    ...
    private static void GlossaryStart(TextView text, String string, String start, String compare) {
